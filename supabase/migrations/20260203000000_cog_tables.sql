...
-- RLS Policies (admin-only tool)
CREATE POLICY "Admin full access to cog_series"
  ON cog_series FOR ALL
  USING (is_admin());

CREATE POLICY "Admin full access to cog_images"
  ON cog_images FOR ALL
  USING (is_admin());

CREATE POLICY "Admin full access to cog_jobs"
  ON cog_jobs FOR ALL
  USING (is_admin());

CREATE POLICY "Admin full access to cog_job_steps"
