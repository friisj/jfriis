  ...
  if (summaries && summaries.length > 0) {
    ...
    layers.push({
      ...
      content: [
        ...
        '',
        'IMAGE GENERATION: You can generate images using the generate_image tool (Gemini Nano Banana models).',
        'Write detailed prompts describing subject, composition, lighting, style, and mood.',
        'Pass reference images from the conversation for style/subject consistency (max 4 via referenceImageUrls).',
        'Use list_prompt_templates to find reusable prompt templates, then pass templateId to generate_image.',
        'Use list_generations to review recent outputs. Default model is Nano Banana 2 (fast); use nano-banana-pro for highest quality.',
        'Available controls: aspectRatio (1:1, 2:3, 3:2, 3:4, 4:3, 9:16, 16:9), imageSize (1K, 2K, 4K).',
      ].join(' '),
