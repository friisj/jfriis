...
- **IMPORTANT**: Defer actionable tasks to Linear (see Linear Integration below)
...
**Critical principle: Linear is the single source of truth for tasks.**

### Documents vs Linear

| Put in Documents | Put in Linear |
|------------------|---------------|
| Specs, context, rationale | Actionable tasks |
| Architecture decisions | Implementation phases |
| Problem statements | Sub-tasks with details |
| Research findings | Progress tracking |
| API schemas, data models | Deadlines and blocking |

### When Creating Planning Documents

1. **DO include**: Overview, goals, approach, technical details, decisions
2. **DO NOT include**: Implementation checklists with [ ] checkboxes
3. **INSTEAD**: Create a Linear project/issues and link to them from the doc

### Document Template (with Linear)
...
## Technical Details
Schemas, APIs, data flows, etc.

## Linear Project
**Track implementation progress in Linear:**
https://linear.app/oji/project/[project-slug]
...
If a user asks to create a planning doc with implementation tasks:

1. Create the spec document (context, approach, technical details)
2. Suggest creating a Linear project for the tasks
3. Link the Linear project from the document
4. Do NOT embed task checklists in the document

Example response:
> "I've created the spec document at `docs/infrastructure/FEATURE_SPEC.md`.
> For the implementation tasks, I recommend creating a Linear project.
