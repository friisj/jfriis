...
import { generateGreenComplexSpec, type GreenComplexity, type GreenSurfaceSpec } from "@/lib/studio/putt/green-complex-generator";
...
/**
 * Spike 1c.2: Surface Feature Primitives Test Page
 *
 * Visualizes composable heightfield features: base slope, tiers, ridges, swales, crowns, false fronts
 */
export default function FeaturesTestPage() {
  ...
  const [enabledFeatures, setEnabledFeatures] = useState({
    base: true,
    undulation: true,  // Spike 1c.6: Base undulation
    tiers: true,
    ridges: true,
    swales: true,
    crowns: true,
    falseFront: false, // Disabled by default (not implemented yet)
  });

  // Pin flats toggle (Spike 1c.3)
  const [showPinFlats, setShowPinFlats] = useState(false);

  // Slope colors toggle (Spike 2.5) - DEPRECATED, use layerConfig instead
  ...
  // Start/cup placement toggle (Spike 1c.4)
  const [showStartCup, setShowStartCup] = useState(false);

  // SDF visualization toggle (Spike 1c.1 verification)
  const [showSDF, setShowSDF] = useState(false);

  // Contours toggle (Spike 1c.5)
  const [showContours, setShowContours] = useState(false);

  // Ball physics controls (Spike 1b + 1b.5)
  const [showBall, setShowBall] = useState(false);
  const [ballSpeed, setBallSpeed] = useState(1.5); // m/s
  const [ballDirection, setBallDirection] = useState(0); // degrees relative to start→cup
  const [ballKey, setBallKey] = useState(0); // for forcing ball reset
  ...
  // Distance override for debugging (null = use generated distance)
  const [distanceOverride, setDistanceOverride] = useState<number | null>(null);

  // Disclosure state for collapsible sections
  ...
  // Cup mechanics controls (Spike 2.5)
  ...
  // Track placement for ball velocity calculation
  ...
  // Final spec with manual overrides applied
  const surfaceSpec = useMemo(() => {
    // Apply undulation overrides if feature is enabled
    ...
    if (distanceOverride !== null && placement) {
      ...
      // Scale positions to achieve new distance (keep cup at same location, move start)
  ...
  // Calculate ball initial velocity from speed and direction
  const ballInitialVelocity: [number, number, number] = useMemo(() => {
    // Direction is relative to start→cup line
    // 0° = straight toward cup, 90° = perpendicular right, -90° = perpendicular left

    // Try to get positions from surfaceSpec first (if explicitly set), otherwise use placement from callback
    const start = surfaceSpec.startPosition || placement?.start;
    const cup = surfaceSpec.cupPosition || placement?.cup;

    if (!start || !cup) {
      // Fallback: use absolute direction (should rarely happen now)
  ...
  const launchBall = () => {
    // Reset ball by changing key
    setBallKey((prev) => prev + 1);
    // Increment attempt count (Spike 2.5)
    if (showCup) {
  ...
  // Reset overrides when seed or complexity changes
  useEffect(() => {
    setBaseSlopeOverride({});
    setUndulationOverride({});
  ...
  // Initialize stimpmeter rating from generated spec (Spike 1b.5)
  useEffect(() => {
  ...
  const metricsPass =
    metrics &&
    metrics.maxSlope <= 8.0 && // Max slope constraint
    metrics.avgSlope <= 5.0; // Reasonable average
    ...
    <div className="h-screen w-screen flex bg-background">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1">
          <Canvas camera={{ position: [0, 25, 35], fov: 50 }} gl={{ antialias: true }}>
            ...
            <GreenSurfaceTest
              ...
              // Legacy props (still supported for backwards compatibility)
        ...
        <footer className="border-t p-3 text-sm bg-card flex gap-8 flex-wrap">
          <div>
            <span className="font-semibold">Controls:</span> Drag to rotate • Scroll to
            zoom • Right-click to pan
          </div>
          <div>
      ...
      <div className="w-96 border-l bg-card overflow-y-auto">
        ...
        {layerConfig.ball.visible && (
              ...
              <div className="space-y-3 text-xs mt-3">
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={() => setBallSpeed(1.0)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      Slow
                    </button>
                    <button
                      onClick={() => setBallSpeed(3.0)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      Medium
                    </button>
                    <button
                      onClick={() => setBallSpeed(6.0)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      Fast
                    </button>
                ...
                <div className="pt-3 mt-3 border-t">
                  ...
                  <div className="flex gap-1 mt-1">
                    <button
                      onClick={() => setStimpmeterRating(7)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      Slow
                    </button>
                    <button
                      onClick={() => setStimpmeterRating(10)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      Medium
                    </button>
                    <button
                      onClick={() => setStimpmeterRating(12)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      Fast
                    </button>
                    <button
                      onClick={() => setStimpmeterRating(14)}
                      className="flex-1 px-2 py-1 text-xs bg-muted hover:bg-muted/80 rounded"
                    >
                      V.Fast
                    </button>
                ...
                {/* Capture Statistics - moved below Launch Ball */}
          ...
          {/* Ball Telemetry Display (Spike 1b) */}
                ...
                <div className="space-y-2">
                  {Object.entries(enabledFeatures).map(([feature, enabled]) => (
                    <div key={feature}>
                      ...
                      {/* Base Slope Controls - shown when Base is enabled */}
                        ...
                        <div className="ml-8 mt-2 space-y-3">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              max="4"
                              step="0.1"
                              value={surfaceSpec.baseSlopePct}
                              onChange={(e) => updateBaseSlopePct(parseFloat(e.target.value))}
                              className="w-full"
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              max="360"
                              step="15"
                              value={surfaceSpec.slopeAngleDeg}
                              onChange={(e) => updateSlopeAngleDeg(parseFloat(e.target.value))}
                              className="w-full"
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              max="0.05"
                              step="0.001"
                              value={surfaceSpec.noiseAmplitude ?? 0.02}
                              onChange={(e) => updateNoiseAmplitude(parseFloat(e.target.value))}
                              className="w-full"
                            />
                      ...
                      {/* Undulation Controls - shown when Undulation is enabled (Spike 1c.6) */}
                        ...
                        <div className="ml-8 mt-2 space-y-3">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              max="1"
                              step="0.01"
                              value={surfaceSpec.undulation.intensity}
                              onChange={(e) => updateUndulationIntensity(parseFloat(e.target.value))}
                              className="w-full"
                            />
                            <p className="text-xs text-muted-foreground mt-1 italic">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              max="10"
                              step="0.5"
                              value={surfaceSpec.undulation.worldScale}
                              onChange={(e) => updateUndulationWorldScale(parseFloat(e.target.value))}
                              className="w-full"
                            />
                            <p className="text-xs text-muted-foreground mt-1 italic">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  max="15"
                  step="0.5"
                  value={falseFrontConfig.rampPct}
                  onChange={(e) =>
                    setFalseFrontConfig((prev) => ({
                      ...prev,
                      rampPct: parseFloat(e.target.value),
                    }))
                  }
                  className="w-full"
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Additional slope percentage at edge
                </p>
              </div>
              <div>
                <label className="text-sm text-muted-foreground block mb-1">
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Vertical drop over ramp distance
                </p>
              </div>
            </div>
          </div>
